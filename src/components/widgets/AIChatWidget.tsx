...
export function AIChatWidget() {
  ...
  const send = () => {
    ...
    const ai: Msg = { role: 'ai', text: cannedReplies[Math.floor(Math.random() * cannedReplies.length)] };
    setMsgs((m) => [...m, user]);
    setInput('');
  ...
  return (
    <>
      <motion.button
        ...
        className="fixed bottom-5 left-5 z-40 h-14 w-14 rounded-full bg-neon-grad shadow-glow flex items-center justify-center text-ink-950"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 left-5 z-40 w-[92vw] max-w-sm glass-strong overflow-hidden flex flex-col"
            style={{ height: 480 }}
          >
            ...
            <div className="p-3 border-t border-white/10 flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm outline-none focus:border-neon-green/50"
              />
