...
function generateCandles(count: number, seed = 42, base = 70000): Candle[] {
  const r = rand(seed);
  const out: Candle[] = [];
  let prev = base;
...
export function CandlestickChart({
  ...
  const [candles, setCandles] = useState<Candle[]>(() => generateCandles(count, seed, base));
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%" className="block">
      <defs>
...
export function Sparkline({
  ...
  seed = 1,
  positive = true,
}: { width?: number; height?: number; seed?: number; positive?: boolean }) {
